-- VapeGUI (modified)
-- Removed example modules, added tab UI and basic Toggle/Slider components
...
local function makeDraggable(gui, window)
    gui.InputBegan:Connect(function(inputObj)
        ...
        if (inputObj.UserInputType == Enum.UserInputType.MouseButton1 or inputObj.UserInputType == Enum.UserInputType.Touch) and (inputObj.Position.Y - gui.AbsolutePosition.Y < 40 or window) then
            local dragPosition = Vector2.new(gui.AbsolutePosition.X - inputObj.Position.X,
                gui.AbsolutePosition.Y - inputObj.Position.Y + guiService:GetGuiInset().Y) / scale.Scale
            local changed = inputService.InputChanged:Connect(function(input)
                if input.UserInputType == (inputObj.UserInputType == Enum.UserInputType.MouseButton1 and Enum.UserInputType.MouseMovement or Enum.UserInputType.Touch) then
                    ...
                    gui.Position = UDim2.fromOffset((position.X / scale.Scale) + dragPosition.X,
...
do
    ...
    if res then
        ...
        uipallet.Font = res.Font and
            Font.new(res.Font:find('rbxasset') and res.Font or
            string.format('rbxasset://fonts/families/%s.json', res.Font)) or
            uipallet.Font
...
local components
components = {
    Divider = function(settings, children, moduleapi)
        -- Support both signatures: (children, text) or (settings, children, moduleapi)
        local text
        if type(settings) == 'string' then
            text = settings
        elseif type(settings) == 'table' and settings.Text then
            text = settings.Text
        end
    ...
    Toggle = function(settings, children, moduleapi)
        local toggle = Instance.new('TextButton')
        toggle.Name = 'ToggleOption'
        toggle.Size = UDim2.new(1, -20, 0, 40)
        toggle.Position = UDim2.new(0, 10, 0, #children:GetChildren() * 42)
        toggle.BackgroundTransparency = 1
        toggle.Text = '   ' .. (settings.Name or '')
        ...
        toggleIndicator.BackgroundColor3 = toggleState and Color3.new(0, 1, 0) or Color3.new(0.3, 0.3, 0.3)
        ...
        local function updateToggle()
            ...
            if settings.Function then
                task.spawn(settings.Function, toggleState)
            end
        end
    ...
    Slider = function(settings, children, moduleapi)
        local sliderValue = settings.Default or settings.Min or 0
        ...
        sliderLabel.Text = '   ' .. (settings.Name or '')
        sliderLabel.Size = UDim2.new(1, 0, 0, 30)
        sliderLabel.BackgroundTransparency = 1
        sliderLabel.TextXAlignment = Enum.TextXAlignment.Left
        sliderLabel.TextColor3 = uipallet.Text
        sliderLabel.TextSize = 24
        sliderLabel.FontFace = uipallet.Font
        ...
        sliderBar.BackgroundColor3 = color.Dark(uipallet.Main, 0.5)
        sliderBar.BorderSizePixel = 0
        sliderBar.Parent = sliderFrame

        local sliderFill = Instance.new('Frame')
        local minv = (settings.Min ~= nil) and settings.Min or 0
        local maxv = (settings.Max ~= nil) and settings.Max or (minv + 1)
        local ratio = (sliderValue - minv) / math.max((maxv - minv), 1)
        sliderFill.Size = UDim2.new(ratio, 0, 1, 0)
        sliderFill.BackgroundColor3 = uipallet.Text
        ...
        return {
            SetValue = function(self, value)
                sliderValue = math.clamp(value, minv, maxv)
                valueLabel.Text = tostring(math.floor(sliderValue))
                local x = (sliderValue - minv) / math.max((maxv - minv), 1)
                sliderFill.Size = UDim2.new(x, 0, 1, 0)
                if settings.Function then
                    task.spawn(settings.Function, sliderValue)
                end
...
function mainapi:CreateCategory(categorysettings)
    ...
    function categoryapi:CreateModule(modulesettings)
        ...
        function moduleapi:Expand()
            ...
            tween:Tween(modulechildren, TweenInfo.new(0.4, Enum.EasingStyle.Linear),
        ...
        -- If a tab visibility function exists, update visibility for this newly created module
        if mainapi.UpdateVisibleModules then
            mainapi.UpdateVisibleModules()
        end
...
-- Removed example modules loop so GUI initializes EMPTY (no example modules)
-- for i = 1, 60 do ... end  (deleted)

-- Create tab buttons UI (visual tabs) and visibility update function
local tabFrame = Instance.new('Frame')
tabFrame.Name = 'TabFrame'
tabFrame.Size = UDim2.new(1, -306, 0, 40)
tabFrame.Position = UDim2.new(0, 153, 0, 20)
tabFrame.BackgroundTransparency = 1
tabFrame.Parent = clickgui
...
local tabs = {}
mainapi.CurrentTab = 'Combat'

-- Function to show/hide modules by category
local function updateVisibleModules()
    local cur = mainapi.CurrentTab
    for _, module in mainapi.Modules do
        if module.Object then
            module.Object.Visible = (module.Category == cur)
        end
    end
end

-- Expose to mainapi so module creation can trigger it
mainapi.UpdateVisibleModules = updateVisibleModules

-- Create a button for each category
local categoryOrder = { 'Combat', 'Blatant', 'Render', 'Utility', 'World', 'Inventory', 'Minigames' }
for _, catName in ipairs(categoryOrder) do
    if mainapi.Categories[catName] then
        local btn = Instance.new('TextButton')
        btn.Name = 'Tab_' .. catName
        btn.Size = UDim2.new(0, 100, 1, 0)
        btn.BackgroundTransparency = 1
        btn.Text = catName
        btn.TextSize = 22
        btn.FontFace = uipallet.Font
        btn.TextColor3 = (catName == mainapi.CurrentTab) and uipallet.Text or color.Dark(uipallet.Text, 0.35)
        btn.Parent = tabFrame

        btn.MouseButton1Click:Connect(function()
            mainapi.CurrentTab = catName
            -- update button states
            for _, b in ipairs(tabFrame:GetChildren()) do
                if b:IsA('TextButton') then
                    b.TextColor3 = (b.Name == 'Tab_' .. catName) and uipallet.Text or color.Dark(uipallet.Text, 0.35)
                end
            end
